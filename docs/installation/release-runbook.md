...
当前生产环境已经从首尔服务器迁移到新加坡服务器。本文档中的服务器路径、容器名、数据卷和反向代理配置以新加坡服务器为准。
...
部署用户: ec2-user
部署目录: /opt/new-api
反向代理配置: /etc/caddy/Caddyfile
Compose 配置: /opt/new-api/docker-compose.yml
```
...
部署目录: /opt/new-api
应用容器: new-api
应用镜像: xiaotubao/new-api:latest 或明确版本 tag
应用端口: 0.0.0.0:3000 -> container 3000
反向代理: Caddy -> 127.0.0.1:3000
数据库: new-api-postgres（postgres:15）
缓存: new-api-redis（redis:latest）
主要配置: /opt/new-api/docker-compose.yml
覆盖配置: /opt/new-api/docker-compose.override.yml（可选，当前服务器可不存在）
```
...
new-api-postgres:
  /var/lib/docker/volumes/new-api_pg_data/_data -> /var/lib/postgresql/data

new-api-redis:
  /var/lib/docker/volumes/new-api_redis_data/_data -> /data
```

数据库和 Redis 连接信息以 Compose 环境变量为准，文档中只记录脱敏格式：

```text
SQL_DSN=postgresql://root:<postgres-password>@postgres:5432/new-api
...
```text
公网域名/HTTPS -> Caddy -> 127.0.0.1:3000 -> new-api:3000
```

当前 Caddy 中指向 `new-api` 的域名：
...
`sg.000328.xyz` 当前指向同机的 `cli-proxy-api-plus` 服务，端口为 `127.0.0.1:8317`，不属于 `new-api` 应用容器。
...
不要把开发源码、临时构建产物、测试数据库或本地 `.env` 文件长期混放在生产部署目录中。生产文档、工单和聊天记录中不要明文保存 PostgreSQL、Redis 或 `SESSION_SECRET` 的真实密钥。

当前 `docker-compose.yml` 将 PostgreSQL `5432` 和 Redis `6379` 暴露到宿主机公网监听地址。除非维护者明确需要外部直连数据库或 Redis，否则建议改为仅 Docker 内网访问，或至少绑定 `127.0.0.1` 并通过云安全组限制来源。
...
sudo find /opt /srv /var/www /www /home /root -maxdepth 5 \
...
查看应用、数据库和 Redis 的挂载位置：

```bash
docker inspect new-api --format '{{range .Mounts}}{{println .Source "->" .Destination}}{{end}}'
docker inspect new-api-postgres --format '{{range .Mounts}}{{println .Source "->" .Destination}}{{end}}'
docker inspect new-api-redis --format '{{range .Mounts}}{{println .Source "->" .Destination}}{{end}}'
```
...
docker inspect new-api --format '{{range .Config.Env}}{{println .}}{{end}}' \
  | grep -Ei '^(PORT|SQL_DSN|LOG_SQL_DSN|REDIS_CONN_STRING|SESSION_SECRET)=' \
  | sed -E 's#(postgresql://[^:]+:)[^@]+#\1****#; s#(redis://:)[^@]+#\1****#; s#(SESSION_SECRET=).*#\1****#'
```
